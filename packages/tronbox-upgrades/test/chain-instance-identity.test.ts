/**
 * The chain layer's instance-identity check: the height-prefix hazard,
 * canonicalization, the transport-fault cross-check, the comparator's
 * totality, and the required-method table's eighth entry.
 *
 ...
 *
 * The total-comparator invariant's other half is negative and is asserted by
 * scan in `chain-absence-scans.test.ts`: no branch anywhere in
 * `src/chain/**` decides that a chain is a development, disposable or local
 * node.
 */
...
// ---------------------------------------------------------------------------
// 1. The height prefix, and why a prefix compare is a wrong answer
// ---------------------------------------------------------------------------

describe('block 1 hashes share their leading eight bytes across every chain', () => {
  ...
  it('pins the real mainnet pair, so a fixture drifting from the chain is visible', () => {
    // The one fixture the block-1 hash-prefix collision finding's own test
    // plan asks for by name. Measured live on
...
// ---------------------------------------------------------------------------
// 2. Canonicalization on both sides
// ---------------------------------------------------------------------------

describe('both sides are canonicalized, so casing is never a false refusal', () => {
  it('compares 0X2B6653DC and 0x2b6653dc as the same chain', () => {
    ...
    // different network", the strongest claim the chain layer makes — for a
    // chain that has not changed at all. A false refusal on the strongest
    // wording is how a correct safety feature gets switched off by its users.
    const verdict = compareChainInstance(
  ...
  it('canonicalizes the *recorded* side too, not only the observed one', () => {
    ...
    // crossed a persistence boundary and may have been written by an older
    // version of the record layer.
    expect(
      compareChainInstance(
...
// ---------------------------------------------------------------------------
// 3. The cross-check is a TRANSPORT fault, not an instance change
// ---------------------------------------------------------------------------
...
describe('a genesis/chain-id disagreement is a transport inconsistency', () => {
  ...
  it('does not read block 1 once the cross-check has failed', async () => {
    // Ordering matters for the round-trip-count assertion and for the
    // diagnosis: a third round-trip against an endpoint already known to be
    // incoherent buys nothing.
    const provider = blockResponder(MAINNET_CHAIN_ID, {
      '0x0': { hash: '0x00000000000000000000000000000000000000000000000000000000cd8690dc' },
    });
  ...
  it('accepts the coherent mainnet pair, so the cross-check refuses nothing real', async () => {
    ...
    // The round-trip-count assertion: exactly three round-trips.
  ...
  it('accepts result: null for block 1 as an answer, not a failure', async () => {
    ...
    // `{"result":null}`, so "there is no block 1" is a **result** and the
    // named-reason invariant forbids collapsing it into the same value as
    // "the read failed".
    const provider = blockResponder(MAINNET_CHAIN_ID, {
      '0x0': { hash: mainnetGenesisHash },
    });
...
// ---------------------------------------------------------------------------
// 4. The total-comparator invariant — total, pure, and indeterminate never refuses
// ---------------------------------------------------------------------------

describe('the comparator is total over {absent, partial, complete}', () => {
  ...
  it('reports indeterminate for the reverse direction — recorded null, observed a hash', () => {
    ...
    // constant across a TRE wipe. So it takes the total-comparator invariant's
    // non-refusing branch.
    expect(
      compareChainInstance(
  ...
  it('is pure — the same inputs give the same answer and nothing is mutated', () => {
    ...
    // The comparator performs no read, so its verdict cannot depend on
...
// ---------------------------------------------------------------------------
// 5. The manifest-path naming invariant — the file the refusal has to be able to name
// ---------------------------------------------------------------------------

describe('manifestPathFor names the file no user would guess', () => {
  ...
  it('refuses a non-hex chain id rather than naming unknown-NaN.json', () => {
    // The guard is re-applied rather than assumed, because the record layer
    // calls this with a value that crossed a persistence boundary. Without it
    // the refusal would cite `.openzeppelin/unknown-NaN.json` — a file no run
    // consults — as the remedy.
