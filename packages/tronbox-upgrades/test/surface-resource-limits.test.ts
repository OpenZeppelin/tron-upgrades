...
/**
 * The option/result surface's Resource Limits & Rate — bounded note
 * recording, bounded traversal of caller data, and a synchronous, I/O-free
 * surface.
 *
 ...
 *
 * 1. **Bounded note recording** — how many degraded notes one channel will
 *    hold, and whether the truncation is honest about how many it dropped.
 * 2. **Bounded traversal of caller data** — how much of the caller's own data
 *    the resolver will walk. `constructorArgs` is arbitrary caller data, so
 *    the bound is *one level* and the detector is a `Proxy` that counts every
 *    read it is subjected to.
 * 3. **Synchronous, I/O-free surface** — that nothing here is asynchronous or
 *    does I/O, driven by running every public entry point with the ambient
 *    world replaced by throwing stubs.
 *
...
// ---------------------------------------------------------------------------
// Bounded note recording
// ---------------------------------------------------------------------------

describe('`recorded` is bounded, and truncation is never silent', () => {
  ...
  it('bounds the buffer the engine relay feeds, so an undeduped upstream cannot grow it', () => {
    /*
     * The note-cap's violation scenario is upstream-driven, not caller-driven.
     * Verified present at `@openzeppelin/upgrades-core@1.46.0`:
     * `dist/utils/log.js:log` writes on **every** call with no dedupe, and
     * `validate` emits one warning per offending struct. A large project
     * therefore relays hundreds of near-identical notes through one window,
     * and they are then held on the returned result for the caller's
     * lifetime and re-emitted on every migration of a `tronbox test` replay.
     */
    const channel = createOutputChannel(channelFacts(noopSink()));
...
// ---------------------------------------------------------------------------
// Bounded traversal of caller data
// ---------------------------------------------------------------------------

/**
 * A `Proxy` that counts every operation performed on it and throws on any read.
 *
 * This is the one-level-traversal bound's detector, and it is the only
 * instrument that can distinguish "does not deep-walk" from "happens not to
 * have deep-walked this fixture": a length-or-index read of an element
 * registers as a count, and a *validating* traversal — `JSON.stringify`, a
 * deep clone, a deep freeze — cannot avoid one.
 */
  ...
  const value = new Proxy(
    {},
    {
      get(_target, property): never {
        counts.gets += 1;
        throw new Error(
          `bounded traversal violated: the resolver read "${String(property)}" off a ` +
...
describe('resolution is bounded by the caller key count and never traverses caller data', () => {
  ...
  it('renders `received` within its budget and never throws doing it', () => {
    ...
    // quoting the option-error message contract wants, and the budget is
    // measured on the quoted form. Stated exactly rather than loosely,
    // because "contains some number" would pass on a renderer that reported
    // the *truncated* length and hid how much was dropped.
    ...
    // the render-budget rule requires the renderer itself never to throw, and
    // the message still says what the value was.
  ...
  it('keeps the budget when the bounded rendering reaches a real error message', () => {
    ...
    // rule, not a data-loss rule.
    expect((thrown as OptionValueError).received).toBe(long);
  });
});

// ---------------------------------------------------------------------------
// Synchronous, I/O-free surface
// ---------------------------------------------------------------------------
...
function withHostileAmbient<T>(run: () => T): T {
  ...
  const refuse = (what: string) => (): never => {
    throw new Error(`synchronous, I/O-free surface violated: ${what} was reached.`);
  };
  try {
    ...
    process.env = new Proxy(
      {},
      {
        get(_target, property): never {
          throw new Error(
            `synchronous, I/O-free surface violated: process.env.${String(property)} was read.`,
          );
        },
      },
...
describe('the channel and the resolver are synchronous and perform no I/O', () => {
  ...
  it('declares no `Promise` return type anywhere in the three directories', () => {
    /*
     * The type-level half, and what makes the thenable-refusal guard
     * meaningful: the one place a promise can appear is a *caller's thunk*,
     * so if any declared return type in these directories were a `Promise`,
     * the guard would be guarding a hole the package had already opened for
     * itself.
     */
  ...
  it('declares no `async` function and contains no `await` in the three directories', () => {
    // The synchronous-capture-window rule's static half, asserted over all
    // three directories rather than only `engine.ts`: an `async` helper
    // anywhere in the package that a window's thunk reached would reopen the
    // hazard from a different file.
  ...
  it('refuses on a real ambient read, so the hostile harness is not vacuous', () => {
    ...
    ).toThrow(/synchronous, I\/O-free surface violated: Date\.now was reached/);
    expect(() =>
      withHostileAmbient(() => {
        return process.env['HOME'];
      }),
    ).toThrow(/synchronous, I\/O-free surface violated: process\.env\.HOME was read/);
  ...
  it('records notes in a bounded, synchronous sequence even when the sink is slow-shaped', () => {
    /*
     * The synchronous/no-I/O rule's violation scenario is that an async
     * channel would make the record-before-write ordering untestable and let
     * a note be recorded after the result was already returned. Asserted
     * directly: the record exists by the time `degraded` returns, on the
     * same tick, with no microtask in between.
     */
    const order: string[] = [];
