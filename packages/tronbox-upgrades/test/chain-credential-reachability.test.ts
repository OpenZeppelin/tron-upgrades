/**
 * The credential-reachability rule, with the timeout-preservation rule's
 * half that depends on it.
 *
 * **The highest-stakes row in the chain layer**, and the only one whose violation
 * sends a secret to a third party rather than merely producing a wrong answer.
 *
 * The mechanism is a property of axios, not of the chain layer:
 ...
 * The credential-reachability rule is keyed to the chain layer's own origin
 * comparison instead, and **both halves are probed here** — the
 * different-origin case for the refusal to lend the client, and the
 * same-origin case because a rule that simply never used the handle would
 * break the timeout-preservation rule instead.
 *
...
describe('the derived endpoint goes through the handle\'s own client', () => {
  it('sends through fullNode.request, carrying the user\'s timeout and the instance credential', async () => {
    ...
    // Inheriting here is not a tolerated leak — it is the same origin the
    // user configured, and it is what preserves the timeout-preservation
    // rule's timeout.
  ...
  it('addresses the derived endpoint relatively, so the fixed-relative-path claim is the same measurement', async () => {
    const handle = createCredentialBearingHandle({ host: HANDLE_HOST });

    await createChainAccess(handle.slot, { env: noEnv });

    // A *relative* url is what lets axios resolve against `baseURL` — and it
    // is also exactly the string the derived-endpoint's fixed relative path
    // needs, since `jsonrpc` is not in `_getConsoleLog`'s three-path
    // allow-list.
...
// ---------------------------------------------------------------------------
// 2. A same-origin override still inherits — the credential-reachability
//    rule's second half
// ---------------------------------------------------------------------------

describe('a same-origin override still uses the handle, so the timeout survives', () => {
  it('routes an override with the handle\'s own origin through fullNode.request', async () => {
    ...
    expect(handle.requests).toHaveLength(1);
    // The absolute form is passed, which is precisely the axios behaviour the
    // credential-reachability rule is about — permitted here because the
    // origin *is* the handle's own.
  ...
  it('treats a handle host that does not parse as a different origin — the safe direction', async () => {
    // The rule's purpose is to keep the handle's credentials off a host the
    // chain layer cannot *prove* is the handle's own, so an unparseable host
    // must fail closed.
...
// ---------------------------------------------------------------------------
// 3. A different-origin override carries nothing the chain layer did not
//    construct
// ---------------------------------------------------------------------------

describe('a different-origin override never touches the handle', () => {
  ...
  it('sends exactly one header, and it is the one the chain layer constructed', async () => {
    const handle = createCredentialBearingHandle({ host: HANDLE_HOST });
    const fetchFixture = createFetchFixture();
    vi.stubGlobal('fetch', fetchFixture.impl);
  ...
  it('carries the same JSON-RPC payload as the same-origin transport, so only the credential differs', async () => {
    ...
    // The credential-reachability rule changes the transport, not the
    // request. If the different-origin path also altered the payload, a
    // caller could not reason about the two the same way — and the
    // divergence would be invisible to every other test here.
...
describe('non-vacuity: the leaking implementation is shown to leak', () => {
  it('records the credential when the absolute override is handed to the handle', async () => {
    ...
    // the cheapest delivery mechanism and the one that preserves the timeout,
    // which is exactly why the credential-reachability rule has to be
    // stated. If this case did not record the secret, every assertion in § 3
    // would be vacuous.
    const handle = createCredentialBearingHandle({ host: HANDLE_HOST });
    // `handle.raw` is the deliberate, typed accessor for the forbidden shape.
    // `handle.slot.tronWrap` is the environment seam's `TronWrapHandle` —
    // `{ trx: object }` — which does not surface `fullNode`, and that
    // narrowness is the raw-handle boundary's first defence. A cast here
    // would make the strongest test in the sub-feature depend on the type
...
describe('with no global fetch, a different-origin override is refused', () => {
  it('refuses with ChainEndpointRefusedError naming deps.post as the remedy', async () => {
    ...
    // It must name *which* source supplied the endpoint (the
    // endpoint-descriptor's origin, rendered).
  ...
  it('does not fall back to the handle\'s client — zero requests through it', async () => {
    ...
    // The operative reading of the four-substitutable-seams completeness is
    // explicit that this fallback "IS the leak the credential-reachability
    // rule exists to prevent". A refusal that still issued the probe through
    // the handle would have leaked before it refused.
    expect(handle.requests).toEqual([]);
  });
...
describe('the rule is keyed to origin, not to which source named it', () => {
  ...
  it('names the environment variable in the refusal when fetch is absent', async () => {
    ...
    expect(failure).toBeInstanceOf(ChainEndpointRefusedError);
    const error: Error = failure instanceof Error ? failure : new Error('none');
    // Sending the user to the wrong configuration is the endpoint-descriptor
    // invariant's failure mode.
