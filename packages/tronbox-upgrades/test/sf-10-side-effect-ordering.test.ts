...
/**
 * SF-10 Side-Effect Ordering & Observability — SF-10 INV-23 … INV-37.
 *
 * Technique: sequence interleaving, with an **unguarded twin** for each guard the
 * category rests on. Four guards get one: INV-27's thenable refusal, INV-31's
 * buffer-and-flush-after-restore, INV-32's non-reentrancy and always-restore, and
 * (in `sf-10-augmentation-boundary.test.ts`) the augmentation policy's non-identity
...
afterEach(() => {
  // INV-32 restated as a suite-wide obligation: no test may leave the stub
  // installed, because the next one would silently capture into a dead buffer.
...
/**
 * The window with INV-27's thenable refusal removed, and nothing else changed.
...
/**
 * The window that does what INV-27 exists to make impossible: `await`s inside the
 * swap. This is the direction that fabricates rather than loses.
 */
...
/** The window with INV-32's `finally` removed. */
...
/** The window with INV-32's re-entrancy guard removed. */
...
/** The window that relays **inside** the swap, which INV-31 forbids. */
...
describe('SF-10 INV-23: the degraded record is the guarantee; the write is a courtesy', () => {
  it.each([
     ...
     * all. A design that discharged SC-003 through the log would be broken for the
     * one command that forces a full replay of every migration on every run.
     */
...
describe('SF-10 INV-24: silenceWarnings() suppresses writes only', () => {
  it('runs the three-way matrix and finds recorded identical in all three', () => {
     ...
     * nothing (INV-36) — while `recorded` and the thrown errors are identical in
     * all three.
     */
    const run = (
    ...
    // `describe()` appends — and that difference is itself the evidence for INV-36:
...
describe('SF-10 INV-25: every write goes through the injected sink with a probed method', () => {
  ...
  it('declares exactly one method on LogSink, so sink.warn is unwritable in the package', () => {
    ...
    // own wrapper — turning SC-003's required degraded-mode statement into a crash.
    const sink: LogSink = recordingSink();
    // @ts-expect-error SF-10 INV-25: LogSink declares `log` only, so `warn` is not on the type.
...
describe('SF-10 INV-26: SF-10 touches console at exactly one site and writes to it nowhere', () => {
  ...
  it('touches no console method across every non-engine fixture', () => {
     ...
     * and its own restore-and-swap discipline is INV-32's subject.
     */
  ...
  it('is handed to SF-11 as a mechanical rule the whole package can be scanned with', () => {
    // Extends SF-0 INV-32 from the seam to the whole package: the same instrument
    // applied to `src/environment/**` must find the seam clean too, which is what
    // makes the rule package-wide rather than SF-10's private discipline.
...
describe('SF-10 INV-27: the capture window admits only a non-thenable return, and refuses rather than awaiting', () => {
...
describe('SF-10 INV-28: no await and no async function inside the capture window', () => {
  ...
  it('calibrates the instrument: the words appear in prose, and prose is not syntax', () => {
    // The same class of trap as INV-26's. `engine.ts` explains the hazard in its
    // own doc comments, so a text grep for `await` or `async` returns hits on a
    // module that is provably free of both.
    const engine = sourceNamed('output/engine.ts');
  ...
  it('holds for every module in the three directories, not only for engine.ts', () => {
    // Handed to SF-11 as the static half of the two-part mechanical check.
  ...
  it('is backed at runtime by INV-27, for a caller that violates the same rule', () => {
...
describe('SF-10 INV-29 / INV-30: the warning-capable export set is enumerated, split, and its hole named', () => {
  it('derives the two subsets from the table rather than declaring them separately', () => {
    ...
    // land in neither list — which is INV-30's stated test.
  ...
  it('makes the wrong fix impossible rather than merely discouraged', () => {
    ...
    // INV-27 refuses it outright, which is what keeps the DOCUMENT disposition
    // honest — there is no way to accidentally "handle" the hole.
    const channel = createOutputChannel(channelFacts(recordingSink()));
    for (const entry of uncapturableEngineExports) {
      expect(() =>
...
describe('SF-10 INV-31: no channel write occurs while console.error is swapped', () => {
...
describe('SF-10 INV-32: the swap is non-reentrant and always restored', () => {
  ...
  it('keeps the re-entrancy guard as the one permitted extra module binding', () => {
    // Enumerated at INV-45 rather than discovered here; asserted here because this
    // is the invariant that introduces it.
    const engine = sourceNamed('output/engine.ts');
...
describe('SF-10 INV-33: the relay preserves upstream text and level, and records both levels', () => {
  ...
  it('preserves the level: a Warning is engine-warning and a Note is engine-note', () => {
     ...
     * from validation** is a reduced-fidelity validation statement by SC-003's own
     * definition — `dist/validate/run/initializer.js:getPossibleInitializers`
     * reports *"Reinitializers are not included in validations by default"* through
     * `logNote`. Dropping it is an SC-003 hole; recording it as a warning would
     * mislabel it.
     */
    const { logWarning, logNote } = upstreamLogWriters();
    const channel = createOutputChannel(channelFacts(recordingSink()));
...
describe('SF-10 INV-34: DegradedCode is one closed union and every reduced-fidelity path has a member', () => {
  it('covers every union member with at least one constructed note', () => {
    // SC-003's *"each such path is covered by at least one test"*, made mechanical
    // by iterating data instead of a hand-written list that silently falls behind.
    const channel = createOutputChannel(channelFacts(recordingSink()));
    for (const code of degradedCodes) {
  ...
  it('refuses a code outside the union at compile time', () => {
    // @ts-expect-error SF-10 INV-34: DegradedCode is closed — no sub-feature may invent a member.
...
describe('SF-10 INV-35: DegradedNote fields are total and frozen', () => {
  it.each([
    ...
    /*
     * A note that says a path degraded but not what to do about it satisfies
     * SC-003's letter and defeats its purpose — the user learns something is wrong
     * and has no next step. The note's producer is always plugin code, so this is
     * a plugin defect, which is why the message names the channel's provenance
...
describe('SF-10 INV-36: origin is provenance — no branch reads it, or hostQuietRequested, to decide whether to write', () => {
  ...
  it('is not vacuous, because describe() reads both and every combination differs', () => {
     ...
     * trivially. `describe()` is that site (CD-5), and it is the only place either
  ...
  it('reads hostQuietRequested only inside describe, and origin only there or as provenance', () => {
    ...
    // chain-aware instrument rather than the identifier one INV-26 uses.
     ...
     * the provenance field INV-36's own statement allows, since it is a value a
     * failure message can name rather than a branch. Both sites are asserted so a
...
describe('SF-10 INV-37: a result\'s notes is exactly the channel\'s recorded, in call order', () => {
