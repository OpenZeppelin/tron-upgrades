...
/*
 * The namespaced/base-slot follow-on — namespaced storage in v1: recognised
 * without a second compile, compared member-wise without positions, and
 * STATED on every path.
 *
 ...
 * is pinned by a persisted compile probe run outside this suite, not
...
describe('the position shortfall is real in the reduced mode and absent with a layout', () => {
...
describe('the statement rides the channel on every path that finds a namespace', () => {
