...
/**
 * The SF-2 ladder's fixture kit: the nine upgrade pairs, the real solc output they
 ...
 * host-shaped, and that is the point: SF-2 reads every artifact fact through
 * `artifacts.record()`, so a fixture that had to populate `_json` would be evidence
 * the boundary leaked.
 */
 ...
 * selection TronBox uses, so a compile-arm identity match is F-6 working end to end
 * rather than a fixture agreeing with itself. A fixture that took them from the
 * slot-level compile would still match — the two selections produce identical
 * bytecode — and would prove nothing about which one the host wrote.
 *
