...
/**
 * SF-10's own fixtures. Kept separate from SF-0's helpers on purpose.
 *
 * `helpers/handles.ts`, `helpers/fixture-catalogue.ts`, `helpers/source-scan.ts`
 * and `helpers/locate.ts` are pinned by SF-0's own assertions — `fixture-catalogue`
 * by a `probes.length > 20` floor and two whole-catalogue sweeps, `source-scan`'s
 * `typedInterpolations()` by a single module-level cache scoped to
 * `src/environment/**`. Extending any of them would silently widen an SF-0
 * assertion, so every SF-10 fixture lives here and the SF-0 helpers are only
 * *read*.
 *
 * Everything below is a plain object or a plain function: SF-10 INV-44 says every
 * host dependency is injected, so no fixture needs a TronBox process, a node
 * runtime seam, or SF-0's environment module.
 */
 ...
 * `real-tronbox.test.ts` and `inv-49-host-import-boundary.test.ts`.
 *
 * `@openzeppelin/upgrades-core` is hoisted to the repo root, and SF-0's INV-49
 * scan forbids `createRequire` in `src/` while explicitly exempting `test/` —
 * which is the whole point of the canary pattern SF-10 INV-2 and INV-29 rest on:
 * the fragile deep import lives where breaking it is a signal rather than an
 * outage.
 */
...
/**
 * The version every SF-10 canary claims its facts against.
 *
 ...
 * the `Deployer`'s fallback. SF-10 INV-25's primary subject.
 */
 ...
 * injected sink carrying `warn`. INV-25's probe is what makes this path and the
 * four above behave identically.
 */
...
/** A host sink that throws. INV-23: the write is a courtesy and may not fail an operation. */
 ...
 *
 * Indents by two spaces **per call**, which is why INV-25 requires exactly one
 * `write` per emission — a three-call emission would get three prefixes.
 */
 ...
 * `console.error`.
 *
 * INV-31's whole reason. If the relay ran inside the capture window, this sink
 ...
 *
 * INV-36's behavioural half needs every combination driven against one fixture,
 * because the property is that none of them changes what is written or recorded.
 */
...
/**
 * A well-formed `DegradedNote`, so a test that is about something *else* does not
 * accidentally exercise INV-35's refusals.
 */
 ...
 *
 * Written out rather than derived: SF-10 INV-4's unknown-key check is the thing
 * under test, so deriving the list from the type under test would make the test
 ...
 * `as unknown as` anywhere in the SF-10 suite; this is the single assertion, and
...
// ---------------------------------------------------------------------------
// Source scanning over SF-10's three directories
// ---------------------------------------------------------------------------
...
/** Every module in one of SF-10's directories, relative paths rooted at `src/`. */
...
/** Every module in all three of SF-10's directories. */
 ...
 *
 * SF-10 INV-1's mechanical half needs exactly this distinction: the portable
 * option surface reaches upstream's members by `extends`, so a member name that
 * appears as a property signature *in* `StandaloneOptions` is a local
 * re-declaration, while the same name reached through `extends` is the property
 * holding. `source-scan.ts:interfaceMembers` works on body text a caller already
 * has; this locates the body by name so the assertion cannot drift to the wrong
 * declaration.
 */
...
export interface ThrowSite {
   ...
   * otherwise. SF-10 INV-8 forbids everything except an enumerated class, so the
   * catch-all cases have to be visible rather than filtered out.
   */
 ...
 *
 * SF-10 INV-8's mechanical half — "no path throws a bare `Error`, a string, or an
 * upstream error re-thrown unwrapped" — is a statement about throw *sites*, and a
 * grep for `throw new Error` would miss `throw 'text'`, `throw error` and
 ...
 *
 * SF-10 INV-36's real content is that `origin` and `hostQuietRequested` are never
 * read *in a conditional that controls emission, recording, or degraded-path
 * selection*. An identifier scan cannot say that — the channel legitimately
 ...
 *
 * SF-10 INV-26's stated instrument is *"a source-level scan asserting exactly one
 * `console` occurrence"*, and implementing it as written **fails on correct
 * code**: a comment-stripped token count of `console.` in the three directories
 ...
 *
 * Handles the three shapes SF-10's sources use: a `function` declaration, a named
 * arrow or function expression bound to a `const`, and a method or arrow in an
 * object literal (`degraded(note) { … }`, `get recorded() { … }`).
 */
 ...
 *
 * INV-26 does not only bound the *count* of `console` references; it says all of
 * them are the save-and-swap inside `runCaptureWindow`. A count alone would pass if
 ...
 * INV-26's subject is `console` as an object. INV-36's subject is the opposite: the
 * reads that matter *are* property accesses off the injected `facts`, so they need
 * their own instrument rather than a loosened version of the first one.
 */
 ...
 *
 * The right instrument for SF-10 INV-45's *"not re-exported from `output/index.ts`"*
 * and for INV-48's surface pin. Two weaker instruments are available and both are
 * wrong: a text search for the identifier **fails on correct code**, because
 * `output/index.ts` documents the two deliberate omissions **by name** — that
 * documentation is the record of the decision, so weakening it to satisfy a grep
 * would be backwards. And `Object.keys(module)` sees only the value exports, so it
 * cannot pin a type export at all, and INV-48's surface includes twenty-odd of them.
 */
 ...
 *
 * SF-10 INV-42's enforcement clause is *"only a `get` trap; no `set`,
 * `defineProperty`, `ownKeys` or `getOwnPropertyDescriptor` trap that alters
 * visibility"*, and that is a statement about which traps **exist**, not about what
 * they happen to return. A behavioural check can only observe the traps that were
 * written; this observes the ones that were not.
 */
 ...
 *
 * SF-10 INV-40's type-level half is *"no declared return type in the three
 * directories contains `Promise`"*, and that is a statement about annotations, not
...
export function sourceNamed(relative: string): ScannedSource {
  ...
  if (found === undefined) {
    throw new Error(
      `No SF-10 source module at '${relative}'. Present: ` +
        sf10Sources()
 ...
 * target.
 *
 * This shape is the reason INV-20 exists. `src/components/Contract/contract.js:Contract._static_methods.addProp`
 ...
 *
 * A plain data property would make every INV-20 test pass against a mechanism
 * that **cannot run**, which is the exact failure DEV-1 records.
 */
 ...
 *
 * This is the fixture SF-10 INV-6's test line asks for, and it can only be built on
 * the real host: the divergence it pins is a property of
 ...
 * so the pair is only reachable through `dist/utils/log`. INV-33 says the relay
 * must be tested against upstream's real output rather than a hand-written
 * string, and this is the only way to produce it.
 *
 ...
 * property is exactly why SF-10 keeps its own flag (divergence D-5).
 */
 ...
 *
 * This is the function CD-3 rests on. It is reached from `getErrors` and therefore
 ...
 *
 * INV-33 requires the ANSI strip to be unconditional, and `chalk@4.1.2`
 * auto-detects the terminal — so a recorded note whose text depended on whether a
