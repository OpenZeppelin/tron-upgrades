/**
 * Fixtures for SF-1 (`chain-state-access`).
 *
 * Two design rules, both load-bearing rather than stylistic.
 *
 * **1. The handle fixture models axios, not a convenient stub.** INV-43's whole
 * risk is a property of `axios.create`: an instance applies its `headers` and
 * `auth` to an **absolute** request URL while ignoring `baseURL`. A fixture that
 * only echoed back the url would pass whether or not SF-1 selected the right
 * transport, so {@link createHandleFixture} reproduces the resolution rule and
 * records the headers and auth that *would* have travelled. That is what makes
 * "the credential did not leak" a measurement instead of an assumption.
 *
 * **2. Nothing here needs a live node, and nothing reaches for a global.** INV-46's
 * four seams (`endpointOverride`, `env`, `post`, `probeNativeApi`) are the complete
 * set, so every fixture in this module is passed *in*. SF-1's own test suite is the
 * first consumer that is not TronBox, which makes this file the standing evidence
...
export type RpcAnswer =
   ...
   * axios behaviour for a non-JSON 2xx body (INV-1).
   */
...
/**
 * The default answer table: enough for `createChainAccess` to complete its one
 * capability probe (INV-32) and nothing more, so a test that needs another method
 * has to say so.
 */
 ...
 * method is how a round-trip-count assertion (INV-40) passes while measuring the
 * wrong thing.
 */
 ...
 *
 * The single most reused fixture in the suite: INV-12, INV-20, INV-37, INV-38,
 * INV-39 and INV-40 are all assertions about *what was posted and how often*, and
 * none of them can be written against a transport that does not count.
 */
...
export interface RecordedHandleRequest {
  /** The url as SF-1 passed it — relative for the derived endpoint (INV-36). */
  readonly url: string;
...
/**
 * Structurally `TronWrapHandle` (`{ trx: object }`) plus the one nested object
 * INV-29 permits SF-1 to read. Declared as an interface so assigning it into a
 * `ChainHandleSlot` is a plain structural assignment rather than an excess-property
 * error — and so no cast appears anywhere in this suite.
 */
...
export interface HandleFixture {
   ...
   *
   * `slot.tronWrap` is `TronWrapHandle` — `{ trx: object }` — and deliberately does
   * not declare `fullNode`: that narrowness is INV-30's first defence. So a test
   * that needs to drive the handle's client *directly*, which INV-43's non-vacuity
   * case does, reaches it here rather than casting.
   */
...
export interface HandleFixtureOptions {
   ...
   * TronGrid key, which is precisely why INV-43 is keyed to an origin check rather
   * than to today's measurement that no credential travels.
   */
  readonly headers?: Readonly<Record<string, string>>;
  readonly auth?: { readonly username: string; readonly password: string };
  /** The user's configured timeout. SF-1 must neither lengthen nor shorten it (INV-41). */
  readonly timeout?: number;
 ...
 * Two consequences are reproduced here because INV-43 turns on both:
 *
...
/** A handle whose `fullNode.host` getter raises — SF-0's `'threw'` state. */
 ...
 *
 * `status` is settable so INV-1's `http-status` transport failure has a driver.
 */
...
/** A bare `{ send }` — INV-47's subject: every reader must work with only this. */
...
/** Mainnet's real block-0 and block-1 hashes, measured live (INV-10's fixture). */
