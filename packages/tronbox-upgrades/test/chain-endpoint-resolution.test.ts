/**
 * The chain layer's endpoint-resolution invariants.
 *
 * Everything the endpoint can be, and everything it must be refused for. Two rows
 * carry most of the weight:
 *
 * **The `/tre` refusal.** `tronWrap.send(method, params)` satisfies
 ...
 *
 * **The scrubber.** The credential chain of custody is verified rather
...
// ---------------------------------------------------------------------------
// 1. The /tre refusal — refused at resolution
// ---------------------------------------------------------------------------

describe('the host\'s /tre cheatcode path is refused at resolution', () => {
  it.each([
  ...
  it('cannot produce a /tre derived endpoint, because jsonrpc is always appended', () => {
    ...
    // which neither *is* nor *ends in* `/tre` — the /tre refusal's exact
    // words. The trap the invariant describes is an **override** copied
    // from the cheatcode documentation, and appending the JSON-RPC path to a
    // host is not that trap.
...
// ---------------------------------------------------------------------------
// 2. Only http and https
// ---------------------------------------------------------------------------

describe('the resolved endpoint must be http or https', () => {
  it.each([
...
// ---------------------------------------------------------------------------
// 3. One source, no fallback
// ---------------------------------------------------------------------------

describe('precedence is fixed, resolved once, with no fallback between sources', () => {
  ...
  it('refuses an empty environment variable rather than falling through', () => {
    // the implementation's decided default, and the fixed-precedence rule's
    // constraint at the *source* level: only `undefined` counts as absent. A
    // user who sets the variable and typos it must be told, not silently
    // served from the derived endpoint of a node that happens to answer.
    // That is the sibling's defect in the chain layer's clothes: a blanket
    // catch plus a hardcoded fallback, which reads slots from a local dev
    // chain on a transient failure.
  ...
  it('attempts exactly one distinct URL when every request fails', async () => {
    ...
    // One probe, one attempt, no second URL and no retry.
    expect(attempted).toHaveLength(1);
    // No JSON-RPC round-trip went through the handle — the injected `post` took it.
    // The handle *does* see one GET — the best-effort native-API refinement
    // probe of `wallet/getnowblock` that the single construction-time probe
    // performs, which exists only to choose between
...
// ---------------------------------------------------------------------------
// 4. The descriptor has two fields and both are scrubbed
// ---------------------------------------------------------------------------

const CREDENTIAL_URL =
  'http://alice:s3cr3t@node.internal:8545/jsonrpc?apikey=AKIA#frag';

describe('the descriptor carries no URL and every rendering is scrubbed', () => {
  it('has exactly the two keys describe and origin', () => {
    const descriptor = resolveWith('http://node.internal:8090');
    // A `url` field would be a second place the credential-redaction
    // guarantee can fail, inside the module whose job is to not have one.
  ...
  it('never leaks the raw URL through JSON.stringify of the whole access object', async () => {
    // This is the frozen-composite invariant's half that this file can drive
    // cheaply: the composite holds the URL in
...
// ---------------------------------------------------------------------------
// 5. One module, exactly two property paths
// ---------------------------------------------------------------------------
...
describe('exactly one module reads the handle, along exactly two paths', () => {
  it('reads only fullNode, fullNode.host and fullNode.request across a full resolution', () => {
    ...
    // The environment seam measured that a configured `privateKey` is
    // reachable from the handles it seals, and the chain layer inherits the
    // handle **without** the sealing because its own
  ...
  it('preserves the missing/threw distinction the environment seam built its diagnosis around', () => {
    const missing = ((): Error => {
      try {
  ...
  it('reports a present-but-wrong-type path as invariant-violated, not as missing', () => {
    ...
    // right problem, which is the failure class the chain layer exists to remove.
...
// ---------------------------------------------------------------------------
// 6. One probe, and its wording is the only thing the
//    native-API refinement can change
// ---------------------------------------------------------------------------

describe('one capability probe at construction, with no unprobed variant', () => {
  ...
  it('disowns the seam\'s appended invocation-context clause', async () => {
    ...
    // parenthetical and disowns it rather than asking the environment seam
  ...
  it('lets the native-API probe change only the wording, never the diagnosis', async () => {
    ...
    // **one** deliberate absorption in `src/chain/**`, and it is mandated by
    // the single construction-time probe rather than tolerated — safe
    // precisely because the value is consumed by
  ...
  it('exports no second constructor and no skip option', async () => {
    ...
    // The single construction-time probe means no `createChainAccessUnchecked`,
    // no `skipProbe`, no lazy variant. An
...
// ---------------------------------------------------------------------------
// 7. The request URL never reaches the host's console-log allow-list
// ---------------------------------------------------------------------------

describe('the derived request url is exactly jsonrpc', () => {
  it('never sends one of the three native paths the host extracts logs from', async () => {
    ...
    // through the host's logger, and the chain layer's "emits nothing"
    // guarantee would be false by way of a function the chain layer never calls.
  ...
  it('pins the allow-list itself, so a host change that adds jsonrpc fails a test', () => {
    // A host-owned list the chain layer has no control over, so the pinned
    // copy is the detector rather than the guarantee.
    expect([
      'wallet/triggerconstantcontract',
      'walletsolidity/triggerconstantcontract',
      'wallet/broadcasttransaction',
