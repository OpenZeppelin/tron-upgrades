...
/**
 * SF-10 Idempotency & Retry — SF-10 INV-15 … INV-18.
 *
...
describe('SF-10 INV-15: resolveUpgradeOptions is pure and idempotent, and leaves the caller byte-identical', () => {
  ...
  it('leaves resolveInitializer and requireProxyKind pure as well', () => {
    // Both are on INV-15's `Applies to` list, and both are total functions over
    // primitives — so the property to check is that repeated calls agree and
  ...
  it('reads no ambient state, so a replay resolves to the same value by construction', () => {
    /*
     * The structural half of purity, and the reason SF-10 declares no replay
     * disposition: a default quietly sourced from an environment variable would
     * make a replay resolve differently from the original run with no diagnostic.
     */
...
describe('SF-10 INV-16: arrays are copied inbound to upstream and frozen outbound to the caller', () => {
  ...
  it('copies one level and no deeper, so element identity is preserved', () => {
    // INV-39's rule, observed from INV-16's side: the copy exists to stop
    // upstream's `push`, not to clone caller data. A deep clone would break
    // identity for a caller passing a contract handle as a constructor argument.
...
// ---------------------------------------------------------------------------
// SF-10 INV-16 / CD-3 — the one sanctioned upstream boundary
// ---------------------------------------------------------------------------
...
describe('SF-10 INV-3 / INV-16 / CD-3: engineValidationOptions is the only sanctioned way to reach the engine', () => {
   ...
   * passing `resolved.validation` directly throws, and it throws for exactly the
   * callers who set an expert opt-out.
   *
   * Both invariants are non-negotiable and they are compatible only if the fresh
  ...
  it('would pass vacuously with a non-triggering allowance, which is why that fixture is not used', () => {
     ...
     * `processExceptions` on a **frozen** array succeeds. A CD-3 test written on
     * this fixture would assert that no boundary is needed.
     */
    const { processExceptions } = upstreamOverrides();
    const resolved = resolveAsJavaScriptCaller(
  ...
  it('carries upstream\'s normalized form once, and it does not grow with engine calls', () => {
     ...
     * inside an array the plugin owns, and INV-46 says upstream owns these six
     * defaults by construction. It is harmless downstream (`processExceptions`
     * asks `includes`), and the two properties that matter are asserted here:
     * the caller's array is untouched, and repeated engine calls do not make the
     * resolved value grow.
     */
    const { processExceptions } = upstreamOverrides();
    const callerOwned = { unsafeAllow: [...ACCUMULATING_UNSAFE_ALLOW] };
...
describe('SF-10 INV-17: the silence flag is process-global and monotonic, and a channel does not capture it', () => {
  ...
  it('is re-read after a reset, and the reset is reachable only by deep import', () => {
     ...
     * suite is not. It is deliberately absent from `output/index.ts` (INV-45), so
...
describe('SF-10 INV-18: SF-10 declares no replay disposition, because it changes no state', () => {
  ...
  it('performs no state change of any kind — no filesystem, no chain, no manifest', () => {
    ...
    // A filesystem import anywhere here would also make INV-43's leaf property
    // false and the "no TronBox process" testability claim false with it.
    for (const source of sf10Sources()) {
