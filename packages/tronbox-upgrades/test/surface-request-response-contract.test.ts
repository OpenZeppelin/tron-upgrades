...
/**
 * The option/result surface's Request/Response Contract — the properties
 * governing what a caller may pass in and what the surface guarantees back.
 *
 ...
 *
 * Every `describe` below states its property in full rather than by a bare
 * number, because the environment seam's own suite numbers a separate set of
 * properties over the same range, and the two would otherwise collide in the
 * same vitest report. A cross-reference to one of the environment seam's
 * properties names it explicitly, so a reference is never ambiguous about
 * which suite it means.
 *
...
describe('the option surface composes upstream types and never re-declares a member narrower', () => {
  it('reaches every upstream member by extension, so each alias is assignable to what it composes', () => {
     ...
     * `Assignable<A, B>` helper, matching the shape the environment seam's
     * suite already uses.
    ...
    // could shake out — the same reason the environment seam's suite pairs
    // each of its pins with one.
  ...
  it('refuses a value outside the closed set at compile time, so a typo cannot type-check', () => {
     ...
     */
    // @ts-expect-error 'delegate-call' is not an UnsafeAllowKind; upstream spells it 'delegatecall'.
    const hyphenated: UnsafeAllowKind = 'delegate-call';
    // @ts-expect-error the closed set is not widened to string.
    const widened: UnsafeAllowKind = 'anything-at-all';
    // @ts-expect-error ProxyKind is closed; case is not normalized (the surface's own rule against silently coercing an out-of-set value).
    const wrongCase: ProxyKind = 'Transparent';
    // @ts-expect-error RedeployMode is closed; 'onChange' is the parity target's own bug.
  ...
  it('declares no member on the portable surface whose name collides with an upstream member', () => {
    /*
     * The mechanical half of the rule that the option surface composes
     * upstream types and never re-declares a member narrower, and the reason
     * it is scoped to four declarations rather than to the file:
     * `ResolvedUpgradeOptions` legitimately declares `timeout` and
     * `pollingInterval` because it is the option surface's *own* type,
...
describe('the closed value sets are derived from the installed engine and complete in both directions', () => {
  ...
  it('declares the enumerations readonly and never mutates them from inside the package', () => {
     ...
     */
    // @ts-expect-error `push` is not on the readonly tuple's type, so no call site can compile.
  ...
  it('recovers UnsafeAllowKind through upstream public types, not by restating literals', () => {
     ...
     * assignable back into upstream's own array member. If the option
     * surface ever restated the union as standalone literals, the second pin
     * would still pass and the first would drift — so both are needed.
     */
...
describe('ResolvedUpgradeOptions is total, frozen, and never holds an undefined value it did not declare', () => {
  ...
  it('freezes the result, its arrays and the call object, refusing every mutation attempt', () => {
     ...
     * reaches the frozen array's own `push` and throws, which is the
     * outbound-freezing property's own wording, verbatim: "a push on
  ...
  it('makes an explicit undefined a compile error under exactOptionalPropertyTypes', () => {
    ...
    // @ts-expect-error explicit undefined is not assignable under exactOptionalPropertyTypes (TS2375).
  ...
  it('declares every field required, so "defaults were applied" is a type-level fact', () => {
    ...
    // break — which is the mechanism, and it is the same device used to pin
    // every result type as total, with no optional fields.
...
describe('undefined means absent — but only for a key the operation accepts', () => {
  ...
  it('rejects an accepted-elsewhere key the operation does not accept', () => {
    ...
    // its own list — which is how the rule stays per-operation without the
    // option surface knowing the operation set.
    expect(() =>
...
// ---------------------------------------------------------------------------
// result-type totality
// ---------------------------------------------------------------------------

describe('every result type is total — no optional fields', () => {
  /*
  ...
  it('gives the operations without a transaction a different type, not an optional field', () => {
    ...
    // @ts-expect-error ValidationOutcome declares no `transaction` — the field does not exist to leave undefined.
...
// ---------------------------------------------------------------------------
// TransactionIdentity.hash
// ---------------------------------------------------------------------------

describe('TransactionIdentity.hash is supplied by the plugin, never read from the host accessor', () => {
...
// ---------------------------------------------------------------------------
// the structural minima pins
// ---------------------------------------------------------------------------

describe('the structural minima are pinned, and the pin asserts what is actually true', () => {
  it('pins OutputChannelSlot to HostChannelFacts with no import in either direction', () => {
    /*
     * The pin lives here rather than in `src/` precisely so it creates no
     * import edge: the zero-import-boundary rule makes `src/output/**` import
     * nothing at all, and this assignment is what keeps the duplicated
  ...
  it('records the stronger contract-handle pin as an executable @ts-expect-error, not as prose', () => {
     ...
     * `address` is required on `ContractHandle` and absent from the
     * environment seam's type (TS2741). Left as written, the implementation
     * would either delete the pin — losing the drift protection that is the
     * whole justification for declaring the minima twice — or cast through
     * it, losing the guarantee silently. So the refusal is recorded as a
     * directive the compiler enforces.
     */
    // @ts-expect-error `address` is required on ContractHandle and absent from the environment seam's ContractAbstraction (TS2741).
  ...
  it('pins the weaker form that does hold, and states that it is a loose pin', () => {
     ...
     * almost any object satisfies the target. It still fails if the
     * environment seam renames `ContractAbstraction` or changes its shape
     * incompatibly — which is the drift it was declared to catch — but it is
     * not a tight pin, and the assertion below is what keeps that fact from
     * being forgotten.
     */
