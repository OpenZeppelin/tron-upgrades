/**
 * SF-1 — the `send` boundary: INV-4, INV-11, INV-12, INV-20, INV-38.
 *
 ...
 *
 * INV-4 is the row with the measured harm. `provider.js:getChainId` is
...
// ---------------------------------------------------------------------------
// 1. INV-4 — the measured harm, then the guard
// ---------------------------------------------------------------------------
...
describe('INV-4: parseInt is not a validator, and the numbers are reproduced', () => {
...
describe('INV-4: send refuses a wrong-shaped result, naming the method and what it saw', () => {
  it.each(chainIdNearMisses)(
...
// ---------------------------------------------------------------------------
// 2. INV-4 — the table is five *distinct* predicates, read as data
// ---------------------------------------------------------------------------

describe('INV-4: stringResultMethods is a table of per-method predicates', () => {
  ...
  it('refuses a method with no declared shape as a defect here, not a node fact', () => {
    ...
    // reachable with a bare `{ send }` (INV-47). Asking it for a shape the table
    // does not describe is an SF-1 bug, and it says so rather than waving through.
    const failure = (():
      | ChainResultShapeError
      | Error => {
      try {
...
// ---------------------------------------------------------------------------
// 3. INV-12 / INV-38 — the two refusals, at zero posts
// ---------------------------------------------------------------------------

describe('INV-12: the metadata methods are refused locally, before any request', () => {
...
// ---------------------------------------------------------------------------
// 4. INV-20 — block tags refused uniformly, at zero posts
// ---------------------------------------------------------------------------
...
describe('INV-20: unsupported tags are refused uniformly, because the node is not uniform', () => {
  ...
  it('leaves block-query methods alone — they accept heights', async () => {
    ...
    // INV-20's uniformity clause is scoped to avoid.
...
// ---------------------------------------------------------------------------
// 5. INV-11 — the method sets are data
// ---------------------------------------------------------------------------

describe('INV-11: the required set is eight, and it is a table rather than a switch', () => {
  it('pins all eight, including the one upgrades-core never calls', () => {
    // `eth_getBlockByNumber` is SF-1's own — the engine never calls it — so a
    // later "simplify the required set to what the engine needs" removes exactly
    // the method the instance fingerprint depends on, silently disabling INV-26
    // while every engine-facing test still passes. Pinning it here is the detector.
