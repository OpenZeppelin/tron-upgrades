/**
 * SF-1 — INV-1, INV-2, INV-3, INV-21, INV-23, INV-24, INV-35, INV-37, INV-38,
 * INV-39, INV-41, INV-46, INV-50.
 *
 * The composite as a whole: what it is, what it costs, what it retains, and — in § 8
 * — the one test in this suite that drives the **real** `Manifest.forNetwork` from
 * `@openzeppelin/upgrades-core` against `access.provider`. That is INV-35's claim end
 * to end: both of SF-1's declared refusals are absorbed by upstream, so
 * `ChainMethodRefusedError` reaches no user, and the manifest resolves to the
 * `unknown-<decimal>` file INV-26's message has to be able to name.
 *
 * INV-2 is the row where the defensive instinct is wrong, and § 2 drives the exact
 * upstream assignment: `provider.js:getTransactionReceipt` does
...
// ---------------------------------------------------------------------------
// 1. INV-3 — total, frozen, and holds no handle or URL
// ---------------------------------------------------------------------------

describe('INV-3: ChainAccess is total and frozen, and holds neither handle nor URL', () => {
...
// ---------------------------------------------------------------------------
// 2. INV-2 — the resolved result is mutable, and upstream's assignment succeeds
// ---------------------------------------------------------------------------

describe('INV-2: send resolves the unwrapped result and freezes nothing', () => {
  ...
  it('resolves the result itself, never the envelope', async () => {
    ...
    // The negative INV-1 states: nothing resolving from `send` carries a top-level
    // `error` key, and nothing is the envelope.
  ...
  it('resolves result: null as an answer rather than a failure', async () => {
    ...
    // so "there is no such block" is an answer, and INV-8 forbids collapsing it into
    // the same value as "the read failed".
...
// ---------------------------------------------------------------------------
// 3. INV-1 / INV-21 — the three-member union, and the id is diagnostic only
// ---------------------------------------------------------------------------
...
describe('INV-1 / INV-21: every disposition, and a mismatched id changes nothing', () => {
  it('classifies java-tron\'s measured "id":"null" + -32700 reply as a node error', async () => {
    ...
    // method produces. Under Design's rule that real, well-formed node error would be
  ...
  it('classifies a well-formed result with a wrong id as a result', async () => {
    ...
    const provider = createProvider(createRpcChannel(descriptor, posts.post));
    // Correlation buys nothing here: SF-1 issues one request per round-trip and never
  ...
  it('states the user\'s timeout is the one that applied (INV-41)', async () => {
    const posts = createRecordingPost({
...
// ---------------------------------------------------------------------------
// 4. INV-23 / INV-39 — send memoizes nothing; identity() memoizes exactly once
// ---------------------------------------------------------------------------

describe('INV-23 / INV-39: N sends are N round-trips, and identity() reads once', () => {
...
// ---------------------------------------------------------------------------
// 5. INV-24 / INV-50 — no module state, and not a singleton
// ---------------------------------------------------------------------------

describe('INV-24 / INV-50: two instances in one process resolve two endpoints', () => {
  ...
  it('retains nothing across a thousand sends beyond the monotonic id', async () => {
    ...
    // and, given the endpoint's credential-bearing nature, a second copy of the thing
    // INV-42 keeps in one place.
...
// ---------------------------------------------------------------------------
// 6. INV-41 / INV-46 — the user's timeout, and all four seams substituted
// ---------------------------------------------------------------------------

describe('INV-41: the handle\'s own client — and its timeout — is the one used', () => {
  it('routes the derived endpoint through the fixture carrying the sentinel timeout', async () => {
    ...
    // the user configured. A hardcoded SF-1 timeout would silently override a
    // deliberate choice — shorter, and a slow public node fails mid-migration;
...
describe('INV-46: every surface works with all four dependencies substituted', () => {
  it('drives the probe, a read and identity() with no real handle behaviour', async () => {
    // The seam-completeness test: the *only* real handle behaviour is the two property
    // reads INV-29 permits. Everything else — the environment, the transport, the
    // native-API probe, the endpoint — is injected. SF-1's own test suite is the first
    // consumer that is not TronBox, and this is the assertion that says so.
    const handle = createHandleFixture({ host: 'http://node.internal:8090' });
    let nativeProbes = 0;
    const responder = identityPost();
...
// ---------------------------------------------------------------------------
// 7. INV-37 / INV-38 — ordering, and SF-1's refusal vs the node's
// ---------------------------------------------------------------------------

describe('INV-37: the probe completes before any reader is reachable', () => {
...
describe('INV-38: verifyCapabilities distinguishes SF-1\'s refusal from the node\'s', () => {
  it('reports refusedLocally for both, with zero posts, while the node WOULD answer', async () => {
    ...
    // unavailable" is indistinguishable between "SF-1 refuses this by policy" and
    // "this node happens not to serve it" — and those have opposite implications. The
    // first is a guarantee; the second is a coincidence SF-1 explicitly refuses to
    // depend on.
    const handle = createHandleFixture({ host: 'http://node.internal:8090' });
  ...
  it('leaves identity()\'s memo untouched', async () => {
    ...
    // INV-38: `verifyCapabilities` performs no writes and changes no state on the
...
// ---------------------------------------------------------------------------
// 8. INV-35 — the real Manifest.forNetwork absorbs both refusals
// ---------------------------------------------------------------------------

describe('INV-35: the declared refusals reach no user, driven against the real engine', () => {
  it('resolves a Manifest through access.provider, absorbing both refusals', async () => {
    ...
    // nested try/catch — so SF-1's deliberately terse refusal is never rendered to a
    // user, and SF-1 does not invest it with message quality it does not need.
    //
    ...
    // resolves to `unknown-728126428` — the name INV-26's refusal has to be able to
    // cite because no user would guess it.
