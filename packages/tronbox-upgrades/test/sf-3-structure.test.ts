...
/**
 * SF-3's **structural** half: the absence scans, the closure walk, the face pins and
 * the engine canary.
 *
...
// ---------------------------------------------------------------------------
// INV-24 — the host is reachable from `src/record/**` by no path
// ---------------------------------------------------------------------------

/**
 * INV-24's report, as renderable strings: host-naming specifiers in any loading
 * syntax, **plus** every module-loading call whose specifier is computed.
 *
...
describe('INV-24 — `src/record/**` names the TronBox host in no loading syntax, and computes no specifier', () => {
...
// ---------------------------------------------------------------------------
// INV-25 — the closure of the highest-consequence module
// ---------------------------------------------------------------------------
...
describe('INV-25 — `address.ts` reaches neither the seam nor the engine, and its whole closure is two third-party packages', () => {
...
// ---------------------------------------------------------------------------
// INV-26 — one module names `Manifest`, and it is inside the record layer
// ---------------------------------------------------------------------------
...
describe('INV-26 — only `manifest.ts` reaches the engine\'s `Manifest`, and no module outside the record layer names it', () => {
...
// ---------------------------------------------------------------------------
// INV-27 — the two functions that give the same wrong answer
// ---------------------------------------------------------------------------
...
/**
 * INV-27's report over a scanned tree: every occurrence of either forbidden name as an
 * identifier or inside a dotted access chain, plus the computed-specifier
 * completeness clause — an access chain assembled from a string is invisible to an
 * identifier census.
 */
...
describe('INV-27 — `setProxyKind` and `detectProxyKind` occur in zero imports and zero access chains', () => {
...
describe('INV-29 — `process.env` is read nowhere in `src/record/**`; the one `env` view arrives through the dependency seam', () => {
...
// ---------------------------------------------------------------------------
// INV-32 — the entry module's static import closure
// ---------------------------------------------------------------------------
...
describe('INV-32 — the walker: transitive, static-only, and `typeOnly`-aware', () => {
...
// ---------------------------------------------------------------------------
// INV-19 — exactly one caught-and-not-rethrown site
// ---------------------------------------------------------------------------
...
describe('INV-19 — exactly one caught-and-not-rethrown site in the record layer, and it is the sidecar read', () => {
...
describe('INV-1 / INV-48 — the face is `openRecord` plus four named values, and everything else is unreachable through it', () => {
...
/**
 * INV-1's consumer half: every name a module outside `src/record/` imports from it.
 *
