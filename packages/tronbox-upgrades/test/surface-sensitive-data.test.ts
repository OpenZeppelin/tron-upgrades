...
/**
 * The option/result surface's Sensitive Data Handling — no host-supplied
 * object ever reaches a formatter, and sealing never widens what the host
 * already exposed.
 *
 * Technique 7, leak probing, with the leak surface the option/result surface
 * actually has. The option/result surface signs nothing and reads no
 * configuration, so it never *sources* a credential; what it does handle is
 * **host-supplied objects that carry one**. TronBox's contract abstraction is
 * the concrete case: `Object.keys` on it already includes `_json` — the full
 * artifact, bytecode and source map — and the migration's surrounding config
 * holds the network private key. So the property under test is the
 * structural one stated here: *no host-supplied object is ever passed to a
 ...
 * `console.log`, template interpolation and own-enumerable traversal alike,
 * because none of those is ever handed a handle. Redaction alone would close
 * `JSON.stringify` and nothing else — which is why the tests below assert
 * **absence at the formatter**, not scrubbing at the output.
 *
 * The sealing-neutrality property is the same concern from the opposite
 * side: sealing must not *widen* what the host already exposed. The
 * assertions are equalities between the sealed and unsealed handle rather
 * than absolute key counts, because the property's content is neutrality,
 * not a number.
 *
 * **The one documented echo** is asserted rather than hidden: a caller's own
 * option *value* is named in the refusal, because every refusal message
 * must name the option's received value. See the last test in the block
 * below asserting no credential or host handle is ever formatted.
 */
 ...
 */
const RETAINED_BY_DESIGN = '<the caller\'s own value, retained in full per the option surface\'s messaging rule>';
 ...
 *
 * **`omit` exists because the stated test instrument here is stronger than the
 * property it tests.** The test line asks for *"`util.inspect` of every error
 * and every note is sentinel-free"*, and that is **false against a correct
 * implementation**: every message must make every fact in it reachable as a
 * structured field too, so `OptionValueError.received` holds the caller's
 * value **in full and unrendered**. A caller who passes an object containing
 * their own private key as `constructorArgs` therefore gets that object back
 * on `error.received` — which is their value returned to them, not a
 * credential the plugin sourced. The property's *statement* is precise about
 * this and its test line is not: it names "any error **message**, any
 * `DegradedNote` field, or any relayed engine text". Those three are probed
 * without exception; `received` is enumerated at its call site with the
 * reason, and asserted directly in the last test in the block below.
 */
...
// ---------------------------------------------------------------------------
// no host object ever reaches a formatter
// ---------------------------------------------------------------------------

describe('no credential and no host handle is ever formatted into a message, a note, or a relayed warning', () => {
  ...
  it('renders a non-string engine write by type alone, never by serializing it', () => {
    ...
    expect(channel.recorded.map(note => note.summary)).toEqual([
      ...
      '[symbol]',
      '[function]',
      // The mixed write: upstream's own string is relayed verbatim, text and
      // level preserved — that is upstream's text, not a plugin serialization
      // — and the *host object* beside it is still reduced to its type.
  ...
  it('keeps every message sentinel-free across every enumerated option-failure path', () => {
    ...
    for (const drive of drives) {
      ...
      // `received` is the caller's own value held in full because every message
      // must make every fact in it reachable as a field, and it is the only field
  ...
  it('keeps the two errors constructible-only paths sentinel-free as well', () => {
    // `OptionUnsupportedOnTronError` has no v1 instance and the two engine
  ...
  it('interpolates only plugin-owned values — no host object reaches a template', () => {
    ...
    const sanctioned: readonly string[] = Object.freeze([
      ...
      // The bounded type-only renderer. This is the only route by which a
  ...
  it('echoes a caller\'s own option value verbatim, which is the one place a secret can appear', () => {
     ...
     *
     * This section's subject is credentials the *plugin* has access to — the
     * network private key in the host's config, the mnemonic, the artifact
     * blob. The option surface reads none of them, which is why every
     * assertion above is an absence.
     *
     * What the option surface *does* echo is the caller's own option value,
     * because every refusal must name what was received — and a caller who
     * writes their private key where a proxy kind belongs gets it named back
     * to them. That is not a plugin leak, it is a diagnostic the caller asked
     * for by supplying the value, and redacting it would break that rule for
     * every legitimate case. Asserted here so the boundary is a recorded
    ...
    // remains on `received`, unrendered, which is what that messaging rule
    // asks for.
    let second: unknown;
    try {
      resolveAsJavaScriptCaller(
...
describe('sealing does not widen the handle\'s exposure', () => {
  ...
  it.each(installedVersions)(
    ...
    installName => {
      ...
      // reason the sealing-neutrality property matters more than it looks.
