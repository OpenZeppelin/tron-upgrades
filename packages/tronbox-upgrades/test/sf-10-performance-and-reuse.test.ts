...
/**
 * SF-10 Performance, Scalability & Re-usability — SF-10 INV-43, INV-44, INV-45,
 * INV-47 and INV-48.
 *
 * INV-46 is this category's remaining member and it is a **dependency canary**, so
 * it lives with the other three in `sf-10-engine-canaries.test.ts`: when
 * `@openzeppelin/upgrades-core` moves, one file fails and its name says why. That is
 * the same placement SF-0 chose for `real-tronbox.test.ts`.
 *
 ...
 *   **embedded in a second host** that is not TronBox, with a different logger
 *   shape, a different contract-abstraction shape and a `Map`-backed cache instead
 *   of an object one — driven end to end with no source change. See § A second host.
 */
...
describe('SF-10 INV-43: the three directories import nothing from the package', () => {
...
function secondHost(): SecondHost {
  ...
  const host: SecondHost = {
    ...
    channelFacts(): HostChannelFacts {
      ...
      // `origin` is provenance only (INV-36), so a second host picks whichever
...
/** The whole SF-10 surface, driven once, against whatever host it is handed. */
...
describe('SF-10 INV-44: every host dependency is injected; nothing is reached for', () => {
  it('names no host package by any loading syntax', () => {
    // Reuses SF-0's own INV-49 instrument rather than a second one: it covers
    // `import`, `export … from`, `import x = require()`, `require`,
  ...
  it('declares every seam as a required parameter', () => {
    ...
    // the ones a call site *cannot* omit. INV-19's guard being counted here is the
    ...
    // `registry` defaults to the v1 registry (CD-8), so only `target` is required —
...
describe('SF-10 INV-45: exactly one module-scope mutable binding exists, and it is the silence flag', () => {
  it('holds exactly two across the three directories — the flag and the re-entrancy guard', () => {
    const bindings = sf10Sources()
      ...
      .sort();
    // `activeCall` is the one permitted addition, and INV-45 enumerates it rather
    // than leaving it to be discovered. Two of them and the audit is no longer a
    // list, which is why this is an exact equality.
  ...
  it('keeps the test-only reset off both export surfaces', () => {
     ...
     * instrument mistake INV-26's `console` scan records.
     */
...
describe('SF-10 INV-47: no deployment-shaped option and no address canonicalization enters this surface', () => {
  ...
  it.each(deploymentShaped)(
    ...
    ({ key, value }) => {
      ...
      // The refusal names the accepted set, so a caller who reached for SF-4's
      // surface early learns what this surface *is* rather than that it "failed".
  ...
  it('references no address-canonicalization helper anywhere in the three directories', () => {
    ...
    expect(offending).toEqual([]);
    // Nor by an access chain — SF-3 owns the canonical form, so not even a
    // `tronWeb.address.toHex` read belongs here.
    expect(
  ...
  it('declares result `address` fields as plain `string`, with no branded type', () => {
    ...
    // every operation to SF-3's decision before SF-3 has made it.
...
describe('SF-10 INV-48: the public surface is additive across minors', () => {
  /**
   ...
   * property under test. *Adding* one also fails here — deliberately: INV-48 says an
   * addition is a minor change, and moving this pin is the one-line edit that
   * records the decision. An assertion that only checked for a superset would let a
   * removal through whenever an addition arrived in the same commit.
   */
  ...
  it('pins the type-only surface, which `Object.keys` cannot see', () => {
    /*
     * Most of what SF-11 re-exports is types — the ten per-operation option
  ...
  it('pins every error `code`, because a caller branches on it', () => {
    // The codes *are* the compatibility surface for error handling — INV-8 exists so
    // nothing requires parsing a message, which makes renaming a code a breaking
