...
/**
 * SF-10 dependency canaries — SF-10 INV-2, INV-29, INV-30 and INV-46.
 *
...
describe('SF-10 engine canaries: the version every claim below is stated against', () => {
...
describe('SF-10 INV-2: the closed value sets are derived from the installed engine', () => {
  ...
  it('keeps `errorKinds` off the root, so the deep import is the honest route', () => {
    ...
    // And `processExceptions` likewise — the two facts together are why INV-2 and
    // CD-3 both use a test-only deep import rather than a `src/` one.
...
// ---------------------------------------------------------------------------
// SF-10 INV-29 and INV-30
// ---------------------------------------------------------------------------

describe('SF-10 INV-29 / INV-30: the warning-capable export set, pinned against the installed dist', () => {
  ...
  it('derives the two subsets from the table, so neither can drift', () => {
    ...
    // INV-30: a new async warning-capable export cannot land in neither list.
    expect(
  ...
  it('finds no warning site outside the enumerated set', () => {
    /*
     * **The canary that makes INV-30's DOCUMENT-not-remedy ruling safe rather than
     * lazy.** A new bypass is a new call to `logWarning`/`logNote`, so the instrument
     * is a call-site census over the whole installed `dist` — 125 compiled modules
     * at 1.46.0.
     *
    ...
    expect(
      ...
      // `silenceWarnings` — which the plugin deliberately does not mirror (CD-7).
      ...
      // `getPossibleInitializers`'s reinitializer note — the `'engine-note'` DEV-3
      // exists for.
  ...
  it('writes exactly one argument to `console.error`, which is what the relay parses', () => {
    /*
     * The basis for the relay's single-argument prefix parse (INV-33). `log` builds
    ...
    // The flag the plugin refuses to reach for (divergence D-5): a module-level
    // `let`, never exported, never readable, never resettable.
...
describe('SF-10 INV-46: the defaults table is pinned by a two-sided canary', () => {
  ...
  it('has the four package-owned defaults matching the recorded table', () => {
     ...
     *
     * `timeout` and `pollingInterval` are **live on TRON** (divergence D-1) where the
     * parity target treated them as inert — its own comment says *"not used for
     * Truffle, but include these anyways"* — so a wrong value here is a real
     * confirmation-policy change rather than a dead field.
     */
