...
/**
 * SF-10 Error Semantics — SF-10 INV-8 … INV-14.
 *
 ...
 * precisely the failure INV-11 exists to prevent (a typo reported as a bad
 * default).
 *
...
/**
 * Every error class SF-10 declares, with the code it carries.
 *
...
describe('SF-10 INV-8: every rejection is a typed error from one closed hierarchy with a stable code', () => {
  ...
  it('roots the four option errors in one base SF-11 can re-export', () => {
...
describe('SF-10 INV-9: never coerce — a value outside its accepted set is refused, not repaired', () => {
  /*
  ...
  it.each(refusals)('refuses: $label', ({ supplied, option }) => {
    ...
    // INV-10: the refusal names the accepted alternative, not merely the failure.
    const accepted =
  ...
  it('produces no resolved value the caller did not pass', () => {
    ...
    for (const kind of unsafeAllowKinds) {
      ...
      // Upstream may *add* derived members (that is INV-15's verified aliasing),
      // but it must never drop the one the caller wrote.
...
describe('SF-10 INV-10: every message names the failing input and the alternative, and the fields carry the same facts', () => {
  ...
  it('reaches every fact in every message as a structured readonly field', () => {
    /*
     * SC-006's requirement, made mechanical: for each class, the facts the message
...
describe('SF-10 INV-11: the check order is fixed and enumerated', () => {
  /*
   ...
   * order at all.
   *
   * The implemented order is CD-9's five checks plus defaults — (1) unknown and
   * TRON-refused keys, (2) closed-set values, (3) contradictions, (4) numeric
   * bounds, (5) shapes, (6) defaults — which is INV-11's enumeration with the
   * TRON refusal folded into step 1, because whether a key is admissible at all
   * belongs before any value is examined.
   */
...
describe('SF-10 INV-12: a contradiction between two channels expressing one allowance is refused, not resolved', () => {
  ...
  it.each(pairs)('refuses $label in both orderings, naming both channels', ({
    ...
    for (const supplied of [first, second]) {
      ...
      // INV-10: which one to keep is on the error, not only in the prose.
...
describe('SF-10 INV-13: received is rendered bounded and type-only for non-primitives', () => {
  ...
  it('never serializes a caller value into the message', () => {
    ...
    // only for the string case, where it cannot throw and is the quoting INV-10
    // asks for. A deep walk anywhere would show up as a second call site.
