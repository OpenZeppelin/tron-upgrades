/**
 * SF-3 — non-vacuity fixtures for six guards whose violation still type-checks.
 *
 ...
 *
 * Covered: INV-9 (never truncated), INV-8 (`firstBlockHash` explicit, never omitted),
 * INV-35 (atomic sidecar write), INV-11 (`indeterminate` never refuses, three
 * causes), INV-34 (refusal precedes any write), INV-14 (the refusal names both
...
/** Keeps the leading `bytes` bytes and drops the rest — the shape INV-9 forbids. */
...
describe('INV-9 — truncation refuses on a chain that did not change, and blames the one signal a wipe cannot move', () => {
...
describe('INV-8 — `firstBlockHash` is written as an explicit `null`; omitting it turns the guard off rather than weakening it', () => {
...
describe('INV-35 — the sidecar write is atomic, so no partial file is ever observable', () => {
...
describe('INV-11 — `indeterminate` never refuses, and its three causes are reached by three distinct routes', () => {
  const observed = identityFor(mainnetFirstBlockHash);
...
describe('INV-34 — the refusal happens before any write, with both files byte-unchanged', () => {
...
describe('INV-14 — the refusal names both files, and says that neither deletion alone resets anything', () => {
