...
/*
 * SF-4 against the real installed host — INV-2's whole subject.
 *
 * The queue premise every SF-4 decision leans on has two arms, and a suite
 * that exercises only the started arm measures the working case. So this file
 * drives the REAL `Deployer` through both arms on both supported minors, and
 * verifies the fixture the unit suite uses (`sf-4-deploy-seam.test.ts`)
...
describe.each(installedVersions)('INV-2 against %s', installName => {
...
describe('the premise is version-stable where SF-4 assumes it is', () => {
  ...
  it.skipIf(installedVersions.length < 2)(
    'linker.js DIFFERS across minors — the fact that scopes INV-21 per minor, pinned so its disappearance is loud',
    () => {
      const [a, b] = installedVersions;
      const read = (name: string): Buffer =>
        fs.readFileSync(
