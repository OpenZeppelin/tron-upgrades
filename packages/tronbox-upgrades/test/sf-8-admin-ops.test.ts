...
/*
 * SF-8 — the authority transfer over a recording fake. Irreversibility is the
 * whole risk, so the file's center is INV-1's negative space (nothing sends
 * when the pre-read already answers) and INV-2's verify (success is the
 * chain's answer, never the receipt's).
 */
...
describe('INV-3: a zero admin slot refuses before any send', () => {
...
describe('INV-1: the pre-read answers, nothing sends', () => {
...
describe('the transfer path: one queued step, confirm, verify', () => {
  it('sends transferOwnership through the ProxyAdmin facade and verifies the new owner', async () => {
    ...
    // Confirm precedes the verify read (INV-4): success is gated twice.
  ...
  it('INV-2: a verify read that does not answer the target refuses naming both', async () => {
    const other = '0x1111111111111111111111111111111111111111';
    const fake = buildFake({
      owners: [SENDER, other],
      senderAddress: SENDER,
