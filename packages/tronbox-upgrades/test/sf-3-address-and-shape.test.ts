...
/*
 * SF-3 — the mint chain, the error family, and the upstream canary.
 *
 * Companion to `sf-3-non-vacuity.test.ts` (behavioural fixtures) and
 * `sf-3-structure.test.ts` (AST and absence scans). This file covers what neither
 * does: that the address mint rejects for five *distinguishable* reasons, that
 * `toHex` is never trusted as a validator, and that the one rule SF-3 obeys because
 * of somebody else's code is pinned where that code lives.
 */
...
describe('INV-2 / INV-3 — the brand exists only behind the asserting mint', () => {
  ...
  it('refuses a value that never passed the mint, rather than trusting its shape', () => {
    // `assertCanonicalAddress` is the re-assertion INV-4 requires at call sites. It
    // must refuse a plausible-looking string, not merely a malformed one.
...
describe('INV-4 — `toHex` is never used as a validator; every call re-asserts its result', () => {
  /*
...
describe('INV-17 / INV-18 — five distinguishable causes, each with its own remedy', () => {
  /*
  ...
  it('reaches each specimen cause, and each carries the caller input verbatim', () => {
    for (const [expected, input] of specimens) {
      ...
      // INV-45: `received` is the only field carrying the caller's raw value, and it
...
describe('INV-47 — `address.ts` is synchronously testable with no fixture, no network, no host', () => {
...
describe('INV-10 — a `changed` instance verdict is unrepresentable, not merely unused', () => {
...
describe('INV-28 — the upstream canary: `processProxyKind` still assigns before delegating', () => {
  /*
   * SF-3's rule is *never call `setProxyKind` or `detectProxyKind`*, and that rule is
   ...
   * `tsc -p tsconfig.test.json`. And a constructed `require` is the primitive INV-49
   * bans across `src/`, so keeping it out of the suite avoids teaching the wrong
   * idiom next to a guard that forbids it.
   */
...
describe('INV-49 — `recordCount` is one definition, and it is the number the user is shown', () => {
