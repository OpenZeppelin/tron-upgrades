/**
 * The chain layer's diagnosis and reader invariants: no-answer classification,
 * transport-failure handling, round-trip costs, and the reader surface's thin
 * binding.
 *
 * **The diagnosis-field rule is the row with the trap that reads as a feature.**
 ...
 * should return `undefined`. The tempting fix is to make the chain layer's own
 * error text match. But the `TvmDiagnosis` kind `'reverted'` **does** contain
 * `revert`, so interpolating the diagnosis into `ChainRpcError`'s message —
 * the natural way to make the error self-explaining — would silently perform
 * exactly the translation this rule bans, and would change
 * `callOptionalSignature`'s behaviour without any change to a call site. § 2
 * asserts the diagnosis is a **field** and is absent from the message.
 *
 * **A second row: the permissive default that disables a safety check.** "Out of
...
// ---------------------------------------------------------------------------
// 1. The diagnosis-field rule — the premise, executed
// ---------------------------------------------------------------------------

describe('upstream\'s four substrings miss TRON\'s probe outcomes', () => {
...
// ---------------------------------------------------------------------------
// 2. Diagnosis as a field, absent from the message
// ---------------------------------------------------------------------------

describe('ChainRpcError carries the diagnosis as a field, never in its text', () => {
  it.each([
  ...
  it('reproduces the node\'s own text byte-for-byte', () => {
...
// ---------------------------------------------------------------------------
// 3. Code first, message second, and no permissive default
// ---------------------------------------------------------------------------

describe('classification keys on code first and message only within a code', () => {
  it.each([
  ...
  it('performs no nested traversal of error.error or error.cause', () => {
    // The code-first classification rule forbids a walk, and the reason there
    // is nothing to walk is that
...
describe('unclassified is never a probe outcome', () => {
  it.each([
  ...
  it('raises for out-of-energy instead of returning a no-answer', async () => {
    // The failure the permissive-default rule exists to prevent, driven end
    // to end: a resource condition the user could fix must not silently
  ...
  it('does not let out-of-energy make looksLikeProxyAdmin answer false', async () => {
    // The specific consumer the permissive-default rule names. `false` here would skip a
...
// ---------------------------------------------------------------------------
// 4. The transport-failure rule — a transport failure is never a no-answer
// ---------------------------------------------------------------------------
...
describe('every transport failure raises at every read.ts entry point', () => {
  ...
  it('bounds a 1 MB non-JSON body in the message and states the truncation', async () => {
    ...
    // headers — which error pages do — a leak of whatever the
    // credential-reachability rule was keeping out.
...
// ---------------------------------------------------------------------------
// 5. Code before slots, and the cost proves it
// ---------------------------------------------------------------------------

describe('no-code and reverted stay distinct at all four surfaces', () => {
  it('reports no-code from readProxySlots at a cost of exactly one round-trip', async () => {
    ...
    // `eth_getCode`-first rule, which is why the round-trip-count assertion
    // states it and the probe-ordering guarantee relies on it. One post, not six.
...
// ---------------------------------------------------------------------------
// 6. The stated round-trip counts, per surface
// ---------------------------------------------------------------------------

describe('every surface costs its stated number of round-trips', () => {
...
// ---------------------------------------------------------------------------
// 7. No-answer reasons are named members, never undefined
// ---------------------------------------------------------------------------

describe('every no-answer branch carries a named reason', () => {
  ...
  it('allows exactly one `| undefined` — readUpgradeInterfaceVersion\'s', async () => {
    ...
    // that has no such getter, and it is the only one in the chain layer's surface.
...
// ---------------------------------------------------------------------------
// 8. Every reader works with a bare { send }, and the binding is thin
// ---------------------------------------------------------------------------

describe('the reader surface takes send as a parameter', () => {
  ...
  it('exposes a frozen reader surface', () => {
    const { provider } = providerOver({});
