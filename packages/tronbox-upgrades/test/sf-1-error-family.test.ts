 ...
 * discriminated result that `read.ts` converts to a throw, which means relaxing INV-6
...
function errorSubclasses(): readonly { name: string; ctor: new (...args: never[]) => Error }[] {
  ...
  for (const [name, value] of Object.entries(errors)) {
    if (
      ...
      // The enumeration idiom SF-0's `error-semantics.test.ts:84` established: a
...
describe('Class location (an open default): the two validation classes live in slots.ts and are re-exported', () => {
  ...
  it('constructs all eleven through errors.ts, each an Error with a non-empty message', () => {
    // INV-13, and it is load-bearing rather than tidy:
  ...
  it('carries eleven unique TRON_CHAIN_* codes (INV-19)', () => {
    const codes = [
  ...
  it('adds no member to SF-0\'s three-diagnosis family', () => {
    ...
    for (const name of subclassNames) {
      expect(name.startsWith('Chain'), `${name} is not an SF-1 class`).toBe(true);
    }
...
// ---------------------------------------------------------------------------
// 2. INV-6 — the whole word, and isEmptySlotWord is total
// ---------------------------------------------------------------------------

describe('INV-6: slotToAddress validates the whole word and refuses', () => {
  ...
  it('is 0x-prefix-optional, and that cannot yield a wrong address', () => {
    ...
    // hex, top 12 bytes zero — still both apply to the digits. INV-6's hazard is
  ...
  it('names the slot, the observed word and why, bounded (INV-44)', () => {
    const failure = ((): Error => {
      try {
        slotToAddress('0x');
...
// ---------------------------------------------------------------------------
// 3. INV-7 — Base58 refused rather than forwarded
// ---------------------------------------------------------------------------

describe('INV-7: inbound addresses are canonicalized, and Base58 is refused', () => {
...
// ---------------------------------------------------------------------------
// 4. INV-5 — sameAddress over a casing and length matrix
// ---------------------------------------------------------------------------

describe('INV-5: sameAddress is case-insensitive, 0x-optional and length-checked', () => {
  ...
  it('applies no EIP-55 checksum', () => {
    // SF-1 emits lowercase branded hex; canonicalization is SF-3's, at the record
    // boundary. If SF-1 imposed a checksum, canonicalization would have two homes and
...
// ---------------------------------------------------------------------------
// 5. INV-49 — the literals re-derived from the installed engine
// ---------------------------------------------------------------------------

describe('INV-49: the slot keys are pinned against the engine\'s own derivation', () => {
  ...
  it.each(labels)('derives $label from toEip1967Hash at test time', ({ label, eip }) => {
    ...
    // grepping `dist/` for them returns zero hits. That makes SF-1's literals a
  ...
  it.each([
    ...
    // no `beacon` key, and the compiler saying so is INV-49's asymmetry clause
  ...
  it.each([
    ...
    // INV-49 asks for the selectors to be verified "against a keccak of the fixed
...
// ---------------------------------------------------------------------------
// 6. INV-45's purity clause — same input, same output
// ---------------------------------------------------------------------------

describe('INV-45: the three zero-import modules export pure functions', () => {
