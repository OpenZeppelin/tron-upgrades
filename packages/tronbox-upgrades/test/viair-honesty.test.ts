...
/**
 * The validation ladder's untested-viaIR guard — `viaIR` is recorded as
 * untested, never as refuted or tested.
 *
 * This guard was adopted with its own instrument marked *specified, not yet
 * written*, because at the time `src/validation-input/` did not exist and
 * the scan had an empty subject. The directory exists now, so the scan is
 * written here rather than left as a specification.
 *
 * **This is the one scan in the validation ladder's territory that reads raw
 * text, comments included, and that is deliberate:** the obligation *is*
 * about prose. Everywhere else in this package an AST scan is mandatory,
 * because the environment seam and the chain layer both found greps that
 * fired on the comment explaining the hazard they were checking for
 * (`test/helpers/source-scan.ts:6-33` records that history). Here the
 * comment is the subject.
 *
 * **Territory.** `src/validation-input/**` only — this guard scopes to that
 * directory, the validation ladder's artifact and its docs. The repository
 * contains exactly two live `viaIR: true` settings outside `node_modules/`
 * and `artifacts/`, at
 ...
 *
 * **Why the obligation exists.** A persisted compile probe outside this
 * suite measured the optimizer half of the ceiling claim (profile P1,
 * optimizer off, last N that compiled 360; P2, optimizer on, 160) and
 * **skipped P3, the `viaIR` profile, for budget**. P2's first failure was a
 * *timeout* rather than an out-of-bounds access, so even P1-versus-P2 is not
 * mechanism-identical. So `viaIR` is **untested, not refuted**, in either
 * direction, and reversal is one flag on that same persisted probe: rerun it
 * with the `P3` profile.
 */
...
describe('viaIR is recorded as untested', () => {
  ...
  it('does not reach outside its declared territory', () => {
    // The table in this guard exists because the first person to run this scan
