...
/**
 * Auth boundary for the option/result surface — covering the host-object
 * augmentation policy, the result contract's sealing mechanism, the
 * never-handed-back rule, and safe member registration.
 *
 * The option/result surface has no caller-authorization boundary: it signs
 * nothing, spends nothing and reads no chain. Its trust boundary is the other one
 * this design cares about —
 ...
 *
 * This is the non-vacuity instrument for the host-object augmentation policy, not
 * an alternative implementation: it exists so the tests below can assert that
 * deleting the guard breaks a named set of cases. If this twin behaved the same as
 * the real function on every fixture, the guard would be decorative and the tests
 * asserting it would be proving nothing.
 */
...
// ---------------------------------------------------------------------------
// The host-object augmentation policy
// ---------------------------------------------------------------------------

describe('the host-object augmentation policy — never mutate an instance the host shares', () => {
  ...
  it('makes the guard a required parameter, so a call site cannot omit it', () => {
    ...
    let thrown: unknown;
    try {
      // @ts-expect-error the guard is required — an omitted guard does not compile.
  ...
  it('is the one property-installation site in src/results/**', () => {
    /*
     * The mechanical rule packaging inherits: `Object.defineProperty`,
...
// ---------------------------------------------------------------------------
// The result contract seals by proxying, not by mutating
// ---------------------------------------------------------------------------

describe('the result contract seals by proxying, not by mutating', () => {
  ...
  it('proves the designed mechanism cannot run, executable rather than quoted', () => {
    /*
...
// ---------------------------------------------------------------------------
// The sealed handle is never handed back to the host
// ---------------------------------------------------------------------------

describe('the sealed handle is never handed back to the host', () => {
  ...
  it('creates the proxy at the return boundary, so no earlier plugin path can hold it', () => {
    ...
    // there is no path in the option/result surface that could pass the proxy onward.
...
// ---------------------------------------------------------------------------
// A member is registered as unavailable only if the host actually has it
// ---------------------------------------------------------------------------

describe('a member is registered as unavailable only if the host actually has it', () => {
