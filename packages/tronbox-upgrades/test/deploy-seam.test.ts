 ...
 * `deploy-real-host.test.ts` is what proves the replication: it runs the same
...
// ---------------------------------------------------------------------------
// the owned-promise bridge
// ---------------------------------------------------------------------------

describe('the bridge settles its own promise where a direct await would hang', () => {
...
describe('a transaction hash inherited from a previous run is refused, not reported', () => {
...
// ---------------------------------------------------------------------------
// the confirmation gate, driven by the captured shapes
// ---------------------------------------------------------------------------

/*
 * Every fixture below is the measured shape from a real mainnet transaction
 * (block 85064760; captured and persisted outside this suite), not an
 * invented one — the difference matters because the invented shape is
...
describe('three disjoint verdicts, each from its measured receipt shape', () => {
...
describe('the vacuity canary — an absent verdict field never classifies as success', () => {
...
describe('the gate consults the wait exactly once — reads retry inside it, sends never', () => {
...
describe('one resolution, canonical comparison, mismatch refuses', () => {
...
describe('linking refuses by default and never trusts silence', () => {
