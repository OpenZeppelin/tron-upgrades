/**
 * The chain layer — eleven invariants that make a wrong answer **unrepresentable**.
 *
...
// ---------------------------------------------------------------------------
// The three-member union, and no member carries both
// ---------------------------------------------------------------------------

describe('(type-level): JsonRpcOutcome has three members and no member carries both', () => {
...
// ---------------------------------------------------------------------------
// the ChainAddress brand
// ---------------------------------------------------------------------------

describe('type-level: ChainAddress is a brand a bare string cannot satisfy', () => {
  it('refuses a bare string where a ChainAddress is required', () => {
    // Minted only by the validating functions. If the chain layer emitted un-branded
    // lowercase strings, the gap to the record layer's canonical form would be
    // crossable by assignment and
...
// ---------------------------------------------------------------------------
// six total discriminated unions, one permitted `| undefined`
// ---------------------------------------------------------------------------

describe('type-level: six total unions, and the reason is always a named member', () => {
  ...
  it('names ProbeDiagnosis as exactly the two probe members', () => {
    // The probe-outcome rule as a type: `isProbeOutcome`'s narrowing target is two members, so a
...
// ---------------------------------------------------------------------------
// EndpointDescriptor has exactly two fields, neither URL-shaped
// ---------------------------------------------------------------------------

describe('type-level: the descriptor has exactly describe and origin', () => {
  it('has no url, host or fullHost field', () => {
    ...
    // assign. A `url` field would be a second place the no-credential-leak
    // guarantee can fail, inside the
...
// ---------------------------------------------------------------------------
// TronWrapRpcView is module-private
// ---------------------------------------------------------------------------

describe('type-level: no module outside endpoint.ts holds a typed handle view', () => {
  it('does not export TronWrapRpcView from the directory\'s face', () => {
    ...
    // both cross the seam's face boundary and put a credential-reachable object into a
...
// ---------------------------------------------------------------------------
// the engine gets access.provider, and the handle is not assignable
// ---------------------------------------------------------------------------

describe('type-level: the host handle is not an EthereumProvider', () => {
  ...
  it('keeps the internal seam deliberately loose, which bare-send embedding requires', () => {
    ...
    // the bare-send embedding test impossible to write without a cast.
...
// ---------------------------------------------------------------------------
// the policy tables are readonly
// ---------------------------------------------------------------------------

describe('type-level: the policy tables are not mutable', () => {
  ...
  it('refuses a write to the shape and block-tag tables, at both layers', () => {
    ...
    // the shape-validation failure mode reached by assignment.
...
// ---------------------------------------------------------------------------
// one constructor, four seams
// ---------------------------------------------------------------------------

describe('type-level: one async constructor, four optional seams', () => {
  it('types createChainAccess as the sole route to a ChainAccess', () => {
    ...
    // There is no unprobed variant, and the type surface is where that is
  ...
  it('names exactly the four dependency seams, all optional', () => {
    ...
    // embedding the chain layer in a different host changes configuration, not source.
...
// ---------------------------------------------------------------------------
// refusedLocally cannot be conflated with a node verdict
// ---------------------------------------------------------------------------

describe('type-level: the two facts have different shapes', () => {
...
// ---------------------------------------------------------------------------
// the recorded shape admits a partial record
// ---------------------------------------------------------------------------

describe('type-level: a partially written record is representable', () => {
