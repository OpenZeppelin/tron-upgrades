 ...
 *
 * INV-45: `silenced` is the one module-scope mutable binding in `src/options/**`,
 * `src/output/**` and `src/results/**`. The only other one in the three
 * directories is `engine.ts`'s re-entrancy guard, which INV-45 enumerates as the
 * single permitted addition. Two of them and the audit is no longer a list.
 *
 * Why process-global rather than per-channel (INV-17): `silenceWarnings()` is
 ...
 * in scope, but is backed by this package's own flag rather than by calling it
 * (divergence D-5). Four reasons, all verified against
 ...
 *
 * **What this suppresses and what it cannot** (INV-24): the flag gates emission
 ...
 * D-5's "including its one-time farewell notice". At call time there is no
 * channel to carry it — that is the whole reason the flag is process-global — and
 * writing it to `console` is forbidden outright (INV-26), because TronBox injects
 ...
 * warns a user who silenced to re-check their unsafe flags, and under INV-24 this
...
/**
 * Read by the channel's emitter and by nothing else — INV-24 requires the flag be
 ...
 * after it behave identically (INV-17).
 */
 ...
 * process.
 *
 * INV-45: **not re-exported from `output/index.ts` and not from the package entry
