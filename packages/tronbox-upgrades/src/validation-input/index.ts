/**
 * The validation ladder's face: one function, its types, the declared
 * compiler range, and the three error classes.
 *
 ...
 * and bypass the policy point entirely — and the call-site scan for `policy`
...
/**
 * The refusal's one rendering plus the two invariant classes.
 *
 ...
 * thirty-three messages across the proxy operations, the standalone operations
 * and adoption (forceImport). The other two are on the face because they are
 * the only two things `deriveValidationInput` throws, and a consumer that
 * wants to tell a plugin bug from a user condition needs to be able to name
 * them.
 */
export {
