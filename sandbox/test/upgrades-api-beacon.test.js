...
describe('hre.upgrades API — beacon kind', function () {
  ...
  it('deploys a beacon fleet: one beacon, two proxies, records written', async () => {
    ...
    const a = await upgrades.deployBeaconProxy(beacon, 'BoxV1', [owner.address, 1n]);
    const b = await upgrades.deployBeaconProxy(beaconAddress, 'BoxV1', [owner.address, 2n]);
    ...
    expect(beaconRecord.contract).to.equal('BoxV1');
  ...
  it('one beacon upgrade moves every proxy, preserving per-proxy state', async () => {
    const [owner] = await ethers.getSigners();
    const beacon = await upgrades.deployBeacon('BoxV1');
    const a = await upgrades.deployBeaconProxy(beacon, 'BoxV1', [owner.address, 10n]);
    const b = await upgrades.deployBeaconProxy(beacon, 'BoxV1', [owner.address, 20n]);

    await upgrades.upgradeBeacon(beacon, 'BoxV2');

    const a2 = await ethers.getContractAt('BoxV2', await a.getAddress());
    const b2 = await ethers.getContractAt('BoxV2', await b.getAddress());
  ...
  it('rejects a layout-incompatible beacon upgrade off-chain', async () => {
    const [owner] = await ethers.getSigners();
    const beacon = await upgrades.deployBeacon('BoxV1');
    await upgrades.deployBeaconProxy(beacon, 'BoxV1', [owner.address, 3n]);
    const implBefore = await upgrades.beacon.getImplementationAddress(beacon);

    let error = null;
    try {
      await upgrades.upgradeBeacon(beacon, 'BoxV2Broken');
    } catch (e) {
      error = e;
    }
  ...
  it('routes upgradeProxy on a beacon proxy to a helpful error', async () => {
    const [owner] = await ethers.getSigners();
    const beacon = await upgrades.deployBeacon('BoxV1');
    const a = await upgrades.deployBeaconProxy(beacon, 'BoxV1', [owner.address, 4n]);

    let error = null;
    try {
      await upgrades.upgradeProxy(a, 'BoxV2');
    } catch (e) {
      error = e;
    }
  ...
  it('erc1967 getters agree across kinds', async () => {
    ...
    const t = await upgrades.deployProxy('BoxV1', [owner.address, 5n]);
    ...
    const u = await upgrades.deployProxy('BoxUUPSV1', [owner.address, 6n], { kind: 'uups' });
